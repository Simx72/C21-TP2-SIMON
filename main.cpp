...
void io_bip();
void io_clean();
double io_round(double v, size_t p = 2);
...
size_t recupererNumeroClient(const Banque &b) {
    size_t x = wherex(), y = wherey();
    string question = "Entrez un numero de client (1 - " + to_string(b.cpt) +
                      " ou 0 pour annuler).\nNumero : ";
...
size_t recupererNumeroCompte() {
    string question =
        "Quel compte (1-" + to_string(COMPTES_MAX) + " ou 0 pour annuler) ? ";
...
bool deposerArgent(Banque &b, size_t numeroClient, size_t numeroCompte,
                   double montant) {
...
bool retirerArgent(Banque &b, size_t numeroClient, size_t numeroCompte,
                   double montant) {
    if (numeroClient < b.cpt && numeroCompte < COMPTES_MAX) {
        b.clients[numeroClient].comptes[numeroCompte].solde -= montant;
        return true;
...
void afficherMenu() {
    ...
    textesMenu[(size_t)Cmd::LISTER] =
        "Lister les clients et leur credit actuel";
...
void cmd_ajouter(Banque &b) {
    ...
    for (uint8_t i = 0; i < 3; i++) {
        string question = "Marge de credit du compte #" + to_string(i + 1) +
                          " (Max de " + toArgentStr(MARGE_CREDIT_MAX) + ")";
        client.comptes[i].margeCredit =
            recupererArgent(question, 0, MARGE_CREDIT_MAX);
...
void cmd_deposer(Banque &b) {
    ...
    if (numeroClient > 0) {
        ...
        if (numeroCompte > 0) {
            ...
            bool ok = deposerArgent(b, numeroClient, numeroCompte, montant);
            if (ok) {
                cout << "Argent depose correctement.";
            } else {
...
void cmd_retirer(Banque &b) {
    clrscr();
    cout << "CMD - Faire un retrait dans un compte client";
    printBreaks(3);
    ...
    if (numeroClient > 0) {
        ...
        if (numeroCompte > 0) {
            ...
            double max = getRetraitMaximum(compte);
            cout << "Retrait maximum : " << toArgentStr(max);

            printBreaks(1);
            double montant = recupererArgent("Montant", 0, max);

            bool ok = retirerArgent(b, numeroClient, numeroCompte, montant);
            if (ok) {
                cout << "Argent retire correctement.";
            } else {
                cerr << "Une erreur est survenue.";
            }
            _getch();
...
// clang-format on
    ...
    do {
        ...
        case Cmd::QUITTER:
            cmd_quitter();
            break;
        case Cmd::INCONNUE: /* Pas de commande */
            break;
        }
